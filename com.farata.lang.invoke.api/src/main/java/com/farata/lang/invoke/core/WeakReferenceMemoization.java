...
class WeakReferenceMemoization<K, V> implements Memoization<K, V> {
    ...
    protected Object getOrCreateMutex(final Reference<K> keyRef) {
        ...
        final Object existingMutex = producerMutexes.putIfAbsent(keyRef, createdMutex);
    ...
    private void expungeStaleEntries() {
        for (Reference<? extends K> ref; (ref = queue.poll()) != null; ) {
            ...
            // We may produce by key equals by value to the garbage collected key 
            // obviously, non-equal by value
            synchronized (mutex) {
