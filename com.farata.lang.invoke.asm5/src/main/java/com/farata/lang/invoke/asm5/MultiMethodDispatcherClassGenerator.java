...
public class MultiMethodDispatcherClassGenerator {
	...
	public byte[] generateClass(final Class<?> interfaceClass, final Method samInterfaceMethod, final Class<?> delegateClass, final Collection<Method> implementationMethods) {
		...
		constructorMethod.visitMethodInsn(INVOKESPECIAL, OBJECT_CLASS_NAME, "<init>", "()V", false);
		constructorMethod.visitVarInsn(ALOAD, 0);
	...
	protected void generateDispatcherMethod(final ClassWriter cw, final String implementationClassName, final Class<?> delegateClass, final Method samInterfaceMethod, final Collection<Method> implementationMethods) {
		...
		final Type returnType = samInterfaceMethod.getReturnType() == void.class || samInterfaceMethod.getReturnType() == null ? null : Type.getType(samInterfaceMethod.getReturnType());
		
		final MethodVisitor mv = cw.visitMethod(ACC_PUBLIC, samInterfaceMethod.getName(), Type.getMethodDescriptor(samInterfaceMethod), null, exceptionTypeNamesOf(samInterfaceMethod));
		mv.visitCode();
		...
		mv.visitMethodInsn(INVOKESTATIC, implementationClassName, INDEXER_METHOD_NAME, getIndexerMethodDescriptor(samInterfaceMethod), false);
		...
		for (final Method impleMethod : implementationMethods) {
			...
			if (isStaticMethod) {
				mv.visitMethodInsn(INVOKESTATIC, Type.getInternalName(impleMethod.getDeclaringClass()), impleMethod.getName(), Type.getMethodDescriptor(impleMethod), false);
			} else {
				mv.visitMethodInsn(
					delegateClass.isInterface() ? INVOKEINTERFACE :	INVOKEVIRTUAL, 
					delegateType.getInternalName(), impleMethod.getName(), Type.getMethodDescriptor(impleMethod), delegateClass.isInterface()
				);
			}
	...
	protected void generateIndexerMethod(final ClassWriter cw, final String implementationClassName, final Method samInterfaceMethod, final Collection<Method> implementationMethods) {
		...
		for (int paramIdx = 0; paramIdx < arity; paramIdx++) {
			...
			mv.visitVarInsn(ALOAD, paramIdx);
			mv.visitMethodInsn(INVOKEVIRTUAL, OBJECT_CLASS_NAME, "getClass", "()Ljava/lang/Class;", false);
			mv.visitLabel(endIf);
		...
		for (final Method impleMethod : implementationMethods) {
			...
			for (int paramIdx = 0; paramIdx < arity; paramIdx++ ) {
				...
				mv.visitMethodInsn(INVOKESTATIC, INTROSPECTION_UTILS_CLASS_NAME, "inheritanceDistance", "(Ljava/lang/Class;Ljava/lang/Class;)I", false);
	...
	private static String getIndexerMethodDescriptor(final Method samInterfaceMethod) {
		final Class<?>[] parameterClasses = samInterfaceMethod.getParameterTypes();
		final Type[] parameterTypes = new Type[parameterClasses.length];
		for (int idx = parameterTypes.length - 1; idx >= 0; idx--) {
			parameterTypes[idx] = Type.getType(parameterClasses[idx]);
		}
		return Type.getMethodDescriptor(Type.INT_TYPE, parameterTypes);
	}
